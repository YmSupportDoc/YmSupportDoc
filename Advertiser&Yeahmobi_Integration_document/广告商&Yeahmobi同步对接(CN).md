...
### 2018年12月
#### 一．对接方案说明
&nbsp&nbsp激活数是广告商主衡量转化效果的重要指标，为更加高效的实现和广告主对接，Yeahmobi可匹配广告商提供多种转化归因方式的对接。
...
&nbsp&nbsp流程说明：核对点击通知时Transaction_id是否有进行宏替换，Yeahmobi Transaction_id共计67位，必须按照标准的Postback链接回调，若回调时提示Unknown Transaction_id Format则表示格式非法。

2. 如何判断广告主回调转化成功：

&nbsp&nbsp流程说明：转化回调时返回：success=true;conversion accepted (please check report for final verification result)。